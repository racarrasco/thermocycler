...
double pid_temp=0;
double pid_acc[PROG_LEN] = {100}; //for multiple temperatures that need to be maintained, 
...
void setup() {
  ...
  //Serial.begin(9600);
  //Serial.begin(14400);
  Serial.begin(19200); //Rigo's original is 14400
  //Serial.begin(57600);
  //Serial.begin(115200);
  //Serial.begin(250000);
  ...
  if (cmdtrig==1) {
    if (cmdpointer==cmdcnt) {
      ...
      switch (cmdarr[cmdpointer]) {
        ...
        break;
        case 'K': //precool for a specified time
        turnoffLED();
        turnonFan();
        type = 'K';
        target = 0;
        ...
        case 'O': //control
          ...
          if (pidtrig==0) { //start
            ...
            if (millis()/100-pidt0>param2arr[cmdpointer]*10) {
              ...
              pid_acc[cmdpointer]=pid_acc[cmdpointer]+pid_in;
              pid_val=pid_kp*pid_in+pid_ki*pid_acc[cmdpointer];//+pid_kd*(pid_pin-pid_in);
  ...
  if (Serial.available() > 0) {
    ...
    if (ibuf[0] == char_term) {
      ...
      if (cmd == 'S') { //status display
        ...
        Serial.print(target);
        Serial.print(' ');
        Serial.print(precooltime); //How long the timer was left to precool
        Serial.println();
      }
      if (cmd == 'L') { //log
